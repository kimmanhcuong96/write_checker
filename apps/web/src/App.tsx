      ...
      : sitePath === "/about" || sitePath === "/contact" || sitePath === "/privacy"
        ...
        {workspaceMode === "checker" && <section id="writing-workspace" className="workspace feature-section" aria-labelledby="writing-heading">
          ...
          <form onSubmit={(event) => { event.preventDefault(); void submit(); }}>
            <div className="workspace-heading"><div><p className="eyebrow">{mode === "estimate" ? t("estimate") : t("targeted")}</p><h2 id="writing-heading">{mode === "estimate" ? t("estimatedLevel") : t("targetReview")}</h2></div><span className={words > maximumWords ? "word-count over" : "word-count"}>{words} / {maximumWords}</span></div>
            <label className="sr-only" htmlFor="writing-input">{mode === "estimate" ? t("estimatedLevel") : t("targetReview")}</label><textarea id="writing-input" value={text} onChange={(event) => { setText(event.target.value); resetDraftState(); }} disabled={Boolean(user && submitting)} placeholder={t("placeholder")} aria-invalid={words > maximumWords}/>
